...
class OpenGui(QWidget):
    ...
    def __init__(self, parent):
        ...
        self.experiment_list = QtGui.QListWidget()

        self.config_layout = QHBoxLayout()

        self.experiment_files = self.parent.relay.namespace["core"].experiment_files
        self.update_experiment_files()

        self.session_layout = QtGui.QVBoxLayout()
        self.action_buttons = QtGui.QVBoxLayout()
        self.sessions_list.currentItemChanged.connect(self.load_experiment)
        self.experiment_list.currentItemChanged.connect(self.load_experiment)
        self.config_layout.addWidget(self.experiment_list)
        ...
        self.show()
    def update_experiment_files(self):
        self.experiment_files = sorted(list(set(self.parent.relay.namespace["core"].experiment_files)))
        self.experiment_list.clear()
        for filename in self.experiment_files:
            w = FileWidget(self,filename)
            if w.exists:
                self.experiment_list.addItem(w)
    def load_experiment(self, curr, prev):
        if curr is None:
            return
        self.experiment_list.setEnabled(False)
        self.selected_experiment = self.parent.relay.namespace["core"].open_experiment(curr.text())
    ...
    def showsessions(self, experiment):
        try:
            ...
            sessions = self.selected_experiment.load_sessions()
            ...
            if type(actions) is list and len(actions) > 0:
                for label,kwargs,description in actions:
                    new_button = QtGui.QPushButton( label )
                    self.session_buttons[label] = new_button
                    new_button.clicked.connect(self.action_button_click)
    ...
    def action_button_click(self):
        if self.sender() is not None:
            ...
            actions = self.selected_experiment.get_actions()
            if type(actions) is list and len(actions) > 0:
                for a,k,description in actions:
                    if a == action_name:
                        kwargs = k
            ...
            if ok:
                ...
                ret = self.selected_experiment.run_action(action_name,kwargs=kwargs)
                #ret = self.selected_sandbox.actions[aa]['function'](**kwargs)
            try:
                print(ret.filename)
                print(ret.walk_task_tree)
                ret.save()
                self.parent.open(self.parent.relay.namespace["core"].spawn_session(ret.filename))
                self.showsessions(self.selected_experiment)
            except:
                raise
...
class SilverTaskRunnerThread(QtCore.QThread):
    def __init__(self, runner):
        ...
        self.runner = runner.namespace["self"]
    def run(self):
...
class SilverResultsWidget(QtGui.QSplitter):
    ...
    def __init__(self,main_widget,session_relay,session_widget):
        QtGui.QSplitter.__init__(self,QtCore.Qt.Horizontal)
        self.session_relay = session_relay
        self.session = session_relay.namespace["session"]
...
silver_session = silver.schedule.Session(None,filename='"""+self.session.filename+"""')
silver_session.load()""")
        self.ipython.push_vars(silver_session=self.session)
...
class SilverTaskOverviewWidget(QtGui.QSplitter):
    ...
    def update(self):
        ...
        if self.session is not None:
            ...
            def addTasks(task):
                ...
                me.setData(task["uuid"])
                compl = QtGui.QStandardItem('{}'.format(task.get("status","?")))
            ...
            if self.filter_text is None or self.filter_text.strip() == "":
                self.model.appendRow(recAddTasks(self.session.root_task.to_json()))
            else:
                ...
                for t in tasks:
                    self.model.appendRow(addTasks(t.to_json()))
    ...
    def openTaskMenu(self, position):
        ...
        item = self.session.root_task.find_uuid(item)
        menu = QtGui.QMenu()
        sand = self.session.experiment.get_sandbox()
        task_actions = []
        try:
    ...
    def run(self):
        ...
        runner = self.session_relay.namespace["self"].spawn_runner(next_task.uuid)
        if type(runner) is dict and 'port' in runner:
            print("Opening new relay")
            runner = ZMQFrontendRelay([runner['port']])
        print("runner: "+str(runner))
        if runner is not None:
            self.status.setText('Running...')
            self.run_button.setEnabled(False)
            self.task_runner = SilverTaskRunnerThread(runner)
            ...
            predicted_time = self.session.guess_time(next_task.uuid)
            self.update()
            if predicted_time > 5:
    ...
    def test(self, index):
        ...
        if ind is None:
            ...
            return
        task = item = self.session.root_task.find_uuid(ind.data())
...
class SilverMainWindow(QtGui.QMainWindow):
    ...
    def __init__(self,ports=["5556"]):
        ...
        self.relay = silver.nodes.ZMQFrontendRelay(ports)
    ...
    def open(self, session_relay):
        session = session_relay.namespace["session"]
        print("opening: "+str(session.filename))
        if session.filename in self.open_sessions.keys():
            return
        self.open_sessions[session.filename] = session_relay
        title = session.filename 
        if session is not None:
            title = session.filename
        #self.openWidget.save_last_sessions(session.filename)
        self.tab_widget.addTab(SilverSessionWidget(self,session_relay), title)
