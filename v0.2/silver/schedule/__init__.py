...
class Experiment(object):
    """ Experiment Object that can create sessions """
    ...
    def add_session(self,session):
        ...
        with open(self.source+'_session.json', 'wb') as fp:
            json.dump(list(set(self.sessions)), fp)
    def get_sandbox(self):
        ...
        if self.sand is None:
            self.sand = imp.load_source('tmp.experiment', self.source)
        return self.sand
    def get_actions(self):
        self.sand = self.get_sandbox()
        return [(k,self.sand.actions[k].get('kwargs',{}),self.sand.actions[k].get('description','')) for k in self.sand.actions.keys()]
    def run_action(self,action_name,kwargs={}):
        kwargs['experiment'] = self
        self.sand = self.get_sandbox()
        returned_session = self.sand.actions[action_name]['function'](**kwargs)
        try:
            returned_session.save()
...
class Session(object):
    ...
    def guess_time(self, task):
        if type(task) is str or type(task) is unicode:
            task = self.root_task.find_uuid(task)
...
class Task(object):
    ...
    def find_uuid(self,u):
        f = self.get_tasks(None, lambda x: x.uuid == u)
        if len(f) > 0:
            return f[0]
        return None
